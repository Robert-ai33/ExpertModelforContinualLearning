"""
Task loader for directed expert class-incremental continual learning.

Subgraph construction:
- Each stage has a set of target classes
- Target nodes: all nodes of target classes
- External neighbors: nodes directly connected to target nodes (via directed edges)
- Edges: both endpoints in subgraph, at least one is target node
- Produces both directed and undirected versions of each subgraph
"""
...
class TaskLoader:
    """Task loader that supports directed and undirected subgraphs."""

    def __init__(self, batch_size, dataset, class_splits,
                 train_shots, valid_shots, test_shots):
        self.batch_size = batch_size
        self.dataset = dataset
        self.data = dataset.data
        self.id_by_class = dataset.id_by_class
        self.directed_edge_index = dataset.directed_edge_index
        self.undirected_edge_index = dataset.undirected_edge_index
    ...
    def _create_task_subgraph(self, class_ids):
        """
        Create task subgraph with both directed and undirected edges.

        Rules:
        - Target nodes: all nodes of given classes
        - External neighbors: nodes directly connected to target nodes
        - Keep edges where both endpoints in subgraph AND at least one is target
        - Produces directed, undirected, and undirected+selfloop edge indices
        """
        # Get target nodes
        target_idx = []
        ...
        # Find all neighbors using directed edges
        directed_ei = self.directed_edge_index
        src, dst = directed_ei[0], directed_ei[1]

        src_is_target = target_mask[src]
        dst_is_target = target_mask[dst]
        connected = src_is_target | dst_is_target

        # Collect all connected nodes
        neighbor_nodes = set()
        connected_indices = torch.where(connected)[0]
        ...
        in_subgraph = all_nodes_mask[src] & all_nodes_mask[dst]
        has_target = target_mask[src] | target_mask[dst]
        valid_directed = in_subgraph & has_target
        directed_subgraph_ei = directed_ei[:, valid_directed]

        # === Undirected subgraph edges (no self-loops) ===
        undirected_ei = self.undirected_edge_index
        u_src, u_dst = undirected_ei[0], undirected_ei[1]
        u_in_subgraph = all_nodes_mask[u_src] & all_nodes_mask[u_dst]
        u_has_target = target_mask[u_src] | target_mask[u_dst]
        valid_undirected = u_in_subgraph & u_has_target
        undirected_subgraph_ei = undirected_ei[:, valid_undirected]

        # === Undirected + self-loops (for GCN) ===
        full_ei_sl = self.data.edge_index
        sl_src, sl_dst = full_ei_sl[0], full_ei_sl[1]
        sl_in_sub = all_nodes_mask[sl_src] & all_nodes_mask[sl_dst]
        sl_has_target = target_mask[sl_src] | target_mask[sl_dst]
        is_selfloop = sl_src == sl_dst
        valid_sl = sl_in_sub & (sl_has_target | is_selfloop)
        undirected_sl_ei = full_ei_sl[:, valid_sl]

        return {
            'target_nodes': sorted(list(target_idx_set)),
            'external_neighbors': sorted(list(external_neighbors)),
            'all_nodes': all_nodes_list,
            'directed_edge_index': directed_subgraph_ei,
            'undirected_edge_index': undirected_subgraph_ei,
            'edge_index': undirected_sl_ei,  # undirected + selfloop, for GCN
            'x': self.data.x,
            'y': self.data.y,
        }
    ...
    def get_task(self, task_id):
        ...
        test_dataset_iso = Subset(self.dataset, test_idx_iso)
        test_dataset_joint = Subset(self.dataset, test_idx_joint)
